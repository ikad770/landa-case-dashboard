// js/cases.js – All Cases list + filters + view modal
...
(function(){
  ...
  function buildPageShell(){
    ...
    page.innerHTML = `
      <div class="page-header">
        <div>
          <h2>All Cases</h2>
          <p>Full list of documented cases with filters and deep dive.</p>
        </div>
      </div>

      <div class="panel">
        <div class="filter-bar">
          <input class="input" id="casesSearch" placeholder="Search in SF Case, Subject, Press, System">
  ...
  function applyPendingDashboardFilter(){
    if(window.pendingCasesFilter){
      ...
      if(field==='system' || field==='region' || field==='pressType' || field==='hwSwEc'){
        currentFilters.search = value;
      }
      if(field==='quarter'){
        currentFilters.quarter = value;
      }
  ...
  function passQuarterFilter(c){
    ...
    const key=`${year}-${q}`;
    return key===currentFilters.quarter;
  }

  function renderActiveFilterSummary(){
    const el=document.getElementById('casesActiveFilter'); if(!el) return;
    const pieces=[];
    if(currentFilters.search){
      pieces.push(`<span class="filter-pill"><b>Search:</b> ${esc(currentFilters.search)}</span>`);
    }
  ...
  function renderTable(){
    const tbody=document.querySelector('#casesTable tbody'); if(!tbody) return;
    ...
    if(!rows.length){
      ...
      tr.innerHTML=`<td colspan="8" style="text-align:center; color:var(--muted); padding:10px;">No cases found for current filters.</td>`;
    ...
    modal.innerHTML = `
      <div class="modal-header">
        <div>
          ...
          <div class="meta">
            ${esc(c.press||'')} · ${esc(c.system||'')}${c.subSystem?' / '+esc(c.subSystem):''}<br>
            Region: ${esc(c.region||'—')} · Date: ${esc(c.caseDate||'—')}
          </div>
        </div>
      ...
      <div class="modal-grid">
        <div>
          ...
          ${
            troubleshooting.length
              ? `<ul style="font-size:12px; margin:4px 0 0 16px;">${troubleshooting.map(t=>`<li>${esc(t)}</li>`).join('')}</ul>`
        ...
        <div>
          ...
          <div class="modal-kv"><span>HW/SW/EC:</span><span>${esc(c.hwSwEc||'')}</span></div>
          <div class="modal-kv"><span>Area:</span><span>${esc(c.area||'')}</span></div>
          <div class="modal-kv"><span>Owner:</span><span>${esc(c.owner||'')}</span></div>
          <div class="modal-kv"><span>Tags:</span><span>${esc(c.tags||'')}</span></div>

          <div class="modal-block-title" style="margin-top:8px;">Parts</div>
          ${
            parts.length
              ...
              : `<div class="modal-kv"><span>No parts linked.</span></div>`
          }
    ...
    modal.querySelectorAll('button[data-act="download"]').forEach(btn=>{
      btn.addEventListener('click',()=>{
        ...
        if(/^https?:\/\//i.test(val)){
          ...
          a.href=val; a.download=''; document.body.appendChild(a); a.click(); a.remove();
        }else{
